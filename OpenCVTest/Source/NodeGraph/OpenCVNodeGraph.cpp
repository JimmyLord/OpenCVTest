...
static float GetClosestPointToCubicBezier(int iterations, float fx, float fy, float start, float end, int slices, const ImVec2& P0, const ImVec2& P1, const ImVec2& P2, const ImVec2& P3) 
{
    if( iterations <= 0 )
        return (start + end) / 2;
...
static ImVec2 GetClosestPointToCubicBezier(const ImVec2 & pos, const ImVec2 & P0, const ImVec2 & P1, const ImVec2 & P2, const ImVec2 & P3, int slices, int iterations)
{
...
static bool IsNearBezierCurve(Vector2 position, Vector2 p0, Vector2 cp0, Vector2 cp1, Vector2 p1)
{
    // Bounds check.
...
: EditorDocument( pEngineCore )
{
    ...
    //m_Nodes.push_back( MyNew OpenCVNode( this, 100, "MainTex", ImVec2(40, 50), 0.5f, ColorByte(255, 100, 100, 255), 1, 1 ) );
    //m_Nodes.push_back( MyNew OpenCVNode( this, 200, "BumpMap", ImVec2(40, 150), 0.42f, ColorByte(200, 100, 200, 255), 1, 1 ) );
    //m_Nodes.push_back( MyNew OpenCVNode( this, 300, "Combine", ImVec2(270, 80), 1.0f, ColorByte(0, 200, 100, 255), 2, 2 ) );
    //m_Links.push_back( OpenCVNodeLink( 100, 0, 300, 0 ) );
    //m_Links.push_back( OpenCVNodeLink( 200, 0, 300, 1 ) );
}
...
void OpenCVNodeGraph::DrawGrid(Vector2 offset)
{
...
int OpenCVNodeGraph::FindNodeIndexByID(NodeID nodeID)
{
    for( uint32 nodeIndex = 0; nodeIndex < m_Nodes.size(); nodeIndex++ )
    {
...
OpenCVNodeGraph::OpenCVNodeLink* OpenCVNodeGraph::FindLinkConnectedToInput(NodeID nodeID, SlotID slotID, int resultIndex)
{
    int count = 0;
...
OpenCVNodeGraph::OpenCVNodeLink* OpenCVNodeGraph::FindLinkConnectedToOutput(NodeID nodeID, SlotID slotID, int resultIndex)
{
    int count = 0;
    for( int i=0; i<m_Links.size(); i++ )
    {
...
OpenCVNodeGraph::OpenCVNode* OpenCVNodeGraph::FindNodeConnectedToInput(NodeID nodeID, SlotID slotID, int resultIndex)
{
    int count = 0;
    for( int i=0; i<m_Links.size(); i++ )
    {
...
OpenCVNodeGraph::OpenCVNode* OpenCVNodeGraph::FindNodeConnectedToOutput(NodeID nodeID, SlotID slotID, int resultIndex)
{
    int count = 0;
    for( int i=0; i<m_Links.size(); i++ )
    {
...
uint32 OpenCVNodeGraph::GetNextNodeIDAndIncrement()
{
    return m_NextNodeID++;
}

void OpenCVNodeGraph::AddExistingNode(OpenCVNode* pNode)
{
    m_Nodes.push_back( pNode );
}

void OpenCVNodeGraph::RemoveExistingNode(OpenCVNode* pNode)
{
    m_Nodes.erase( std::find( m_Nodes.begin(), m_Nodes.end(), pNode ) );
}

bool OpenCVNodeGraph::IsNodeSlotInUse(NodeID nodeID, SlotID slotID, SlotType slotType)
{
...
void OpenCVNodeGraph::Update()
{
    ...
    else
    {
        ...
        // Draw the links between nodes.
        {
            ...
            for( int linkIndex = 0; linkIndex < m_Links.Size; linkIndex++ )
            {
                OpenCVNodeLink* pLink = &m_Links[linkIndex];
                OpenCVNode* pOutputNode = m_Nodes[FindNodeIndexByID( pLink->m_OutputNodeID )];
                OpenCVNode* pInputNode = m_Nodes[FindNodeIndexByID( pLink->m_InputNodeID )];
            ...
            if( m_MouseNodeLinkStartPoint.InUse() )
            {
                ...
                OpenCVNode* pInputNode = m_Nodes[inputNodeIndex];
        ...
        // Deal with context menus.
        {
            ...
            if( ImGui::BeginPopup( "context menu" ) )
            {
                if( m_SelectedNodeLinkIndex != -1 )
                {
                    ...
                    {
                        m_pCommandStack->Do( MyNew EditorCommand_OpenCVNodeGraph_DeleteLink( this, m_SelectedNodeLinkIndex ) );
                    }
                }
                else 
                {
                    OpenCVNode* pNode = nullptr;
                    ...
                    {
                        OpenCVNode* pNode = m_pNodeTypeManager->AddCreateNodeItemsToContextMenu( scenePos, this );
                        if( pNode )
                        {
                            m_pCommandStack->Do( MyNew EditorCommand_OpenCVNodeGraph_AddNode( this, pNode ) );
                        }
...
const char* OpenCVNodeGraph::ExportAsLuaString()
{
...
cJSON* OpenCVNodeGraph::ExportAsJSONObject()
{
...
void OpenCVNodeGraph::ImportFromJSONObject(cJSON* jNodeGraph)
{
    ...
    for( int nodeIndex = 0; nodeIndex < cJSON_GetArraySize( jNodeArray ); nodeIndex++ )
    {
        ...
        if( jType )
        {
            ...
            OpenCVNode* pNode = m_pNodeTypeManager->CreateNode( jType->valuestring, Vector2( 0, 0 ), this );
