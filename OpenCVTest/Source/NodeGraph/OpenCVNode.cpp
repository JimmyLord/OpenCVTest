...
OpenCVNodeGraph* OpenCVNodeGraph::OpenCVNode::GetNodeGraph()
{
    return m_pNodeGraph;
}

uint32 OpenCVNodeGraph::OpenCVNode::GetInputSlotCount() const
{
    return m_InputsCount;
}

uint32 OpenCVNodeGraph::OpenCVNode::GetOutputSlotCount() const
{
    return m_OutputsCount;
}

ImVec2 OpenCVNodeGraph::OpenCVNode::GetInputSlotPos(SlotID slotID) const
{
    MyAssert( slotID < m_InputsCount );
    return ImVec2( m_Pos.x, m_Pos.y + m_Size.y * ((float)slotID + 1) / ((float)m_InputsCount + 1) );
}
    
ImVec2 OpenCVNodeGraph::OpenCVNode::GetOutputSlotPos(SlotID slotID) const
{
    MyAssert( slotID < m_OutputsCount );
    return ImVec2( m_Pos.x + m_Size.x, m_Pos.y + m_Size.y * ((float)slotID + 1) / ((float)m_OutputsCount + 1) );
}

void OpenCVNodeGraph::OpenCVNode::Draw(ImDrawList* pDrawList, Vector2 offset, bool isSelected, MouseNodeLinkStartPoint* pMouseNodeLink)
{
    ImGui::PushID( m_ID );
    ...
    if( nodeIsSelected && titleOfNodeIsActive && ImGui::IsMouseDragging(0) && pMouseNodeLink->InUse() == false )
    {
        for( int i=0; i<m_pNodeGraph->m_SelectedNodeIDs.Size; i++ )
        {
            ...
            OpenCVNode* pNode = m_pNodeGraph->m_Nodes[nodeIndex];
...
void OpenCVNodeGraph::OpenCVNode::HandleNodeSlot(ImDrawList* pDrawList, Vector2 slotPos, NodeID nodeID, SlotID slotID, SlotType slotType, MouseNodeLinkStartPoint* pMouseNodeLink)
{
...
bool OpenCVNodeGraph::OpenCVNode::HandleNodeLinkCreation(Vector2 slotPos, NodeID nodeID, SlotID slotID, SlotType slotType, MouseNodeLinkStartPoint* pMouseNodeLink)
{
    ...
    else
    {
        if( pMouseNodeLink->m_SlotType == SlotType_Input && slotType == SlotType_Output )
        {
            ...
            if( ImGui::IsMouseReleased( 0 ) )
            {
                m_pNodeGraph->m_pCommandStack->Do( MyNew EditorCommand_OpenCVNodeGraph_CreateLink( m_pNodeGraph, OpenCVNodeLink( nodeID, slotID, pMouseNodeLink->m_NodeID, pMouseNodeLink->m_SlotID ) ) );
            }
        }
        else if( pMouseNodeLink->m_SlotType == SlotType_Output && slotType == SlotType_Input )
        {
            ...
            if( ImGui::IsMouseReleased( 0 ) )
            {
                m_pNodeGraph->m_pCommandStack->Do( MyNew EditorCommand_OpenCVNodeGraph_CreateLink( m_pNodeGraph, OpenCVNodeLink( pMouseNodeLink->m_NodeID, pMouseNodeLink->m_SlotID, nodeID, slotID ) ) );
            }
        }
        else
        {
...
void OpenCVNodeGraph::OpenCVNode::DrawTitle()
{
    ImGui::Text( m_Name );
}

void OpenCVNodeGraph::OpenCVNode::DrawContents()
{
...
cJSON* OpenCVNodeGraph::OpenCVNode::ExportAsJSONObject()
{
...
void OpenCVNodeGraph::OpenCVNode::ImportFromJSONObject(cJSON* jNode)
{
